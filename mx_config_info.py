...
# DESCRIPTION
# The goal is to pull configuration information from security appliances in a Meraki
# network and then export to an Excel file. Use printOrgs.py to get net_id

# USAGE
# python mx_config_info.py -k <api_key> -o <org_id> -n <net_id>
...
def main(argv):
    ...
    MXs = [device for device in inventory if device['model'][:2] in ('MX') and device['networkId'] is not None]
    print(MXs)
    # For loop to cycle through each MX
    # ----Creates a list of serial numbers and hostnames
    MX_host_serials = []
    net_count = []
    for MX in MXs:
        if MX['serial'] not in MX_host_serials:
            MX_host_serials.append([MX['serial'],MX['name']])

    # ----Creates a list of network IDs; used only for status counter today
        if MX['networkId'] not in net_count:
            net_count.append(MX['networkId'])

    # Prints out a multi-level lists with serial numbers and hostnames
    print (MX_host_serials)
    # Prints total # of switches in organization
    print('Found a total of %d MXs configured across %d networks in this organization.' % (len(MXs), len(net_count)))

    i = 0
    # For loop to iterate through each rows in switch_host_serials
    for row in range(len(MX_host_serials)):

        # Prints a quick status counter, serial number and hostname
        print('Count '+str(i+1)+'/'+str(len(MXs))+':  Gathering data for '+MX_host_serials[i][1]+' - '+MX_host_serials[i][0])
